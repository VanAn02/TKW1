        ...
        <v-table fixed-header height="800px">
            ...
            <tbody>
                <tr v-for="(item, index) in datas" :key="index">
                    ...
                    <td class="text-left text-green" v-else>Đã duyệt</td>
                    <td class="text-left">
                        ...
                        <v-btn v-if="item.TrangThai === 0" color="red" size="x-small" class="mr-3"
                            @click="Duyet(item)">
                            Duyệt
                        </v-btn>
...
import ChiTietDonHang from '@/layout/Client/ChiTietDonHang.vue';
export default {
  components: { ChiTietDonHang },
    ...
    methods: {
        getHoaDonById() {
            axios.get('https://localhost:7125/api/HoaDon').then(res => {
                this.datas = res.data;
        ...
        Duyet(item){
            item.TrangThai=1;
            axios.put('https://localhost:7125/api/HoaDon/' + item.HoaDonId,item).then(res => {
                this.getHoaDonById();
            }).catch(error => {
                console.log(error);
            })
